        ...
        if len(cmd) > 1:
            try:
                ...
                if guess == self.num:
                    ...
                    if guess < self.num:
                        self.send_msg("Your number is too low! Try again!", self.channel)
                    elif guess > self.num:
                        self.send_msg("Your number is too high! Try again!", self.channel)
            except:
                self.send_msg("Please guess a number!", self.channel)
        else:
            self.send_msg("Please guess a number between 1 and 1023!", self.channel)